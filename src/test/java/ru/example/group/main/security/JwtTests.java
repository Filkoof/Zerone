...
public class JwtTests extends AbstractAllTestH2ContextLoad {
    ...
    void jwtLogin() {
        ...
        assertTrue(jwtUtilService.validateToken(socialNetUserRegisterService.jwtLogin(null, null, payload).getData().getToken(), userDetails));
    }
    ...
    void jwtTokenValidationAndExpiredRegisterServiceAndUtilService() {
        ...
        SocialNetUserDetails userDetails =
                (SocialNetUserDetails) socialNetUserDetailsService.loadUserByUsername(payload.getEmail());
        assertTrue(jwtUtilService.validateToken(socialNetUserRegisterService.jwtLogin(null, null, payload).getData().getToken(), userDetails));
        assertTrue(!jwtUtilService.isTokenExpired(socialNetUserRegisterService.jwtLogin(null, null,  payload).getData().getToken()));
    }
