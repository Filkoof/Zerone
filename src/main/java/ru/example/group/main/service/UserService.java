...
public class UserService {
  ...
  public UserService(UserRepository userRepository, ZeroneMailSender zeroneMailSender,
    ...
    try {
      if (!StringUtil.isEmpty(user.getEmail())) {
        String message = String.format(
            "Hello, %s! \n" +
                "Welcome to Sweater. Please, visit next link: http://" + mailHost
                + "/api/v1/account/registration_complete/%s",
            user.getFirstName(),
